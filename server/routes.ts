...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get('/api/search', async (req, res) => {
    try {
      ...
      const [jamendo, rapidapi, soundcloud, youtube, itunes, internetArchive, mixcloud] = await Promise.all([
        (await import('./lib/jamendo')).searchJamendo(query, 10).catch((err) => { console.error('Jamendo error:', err.message); return []; }),
        (await import('./lib/rapidapi-music')).searchRapidApiMusic(query, 10).catch((err) => { console.error('RapidAPI error:', err.message); return []; }),
        (await import('./lib/soundcloud')).searchSoundCloud(query, 10).catch((err) => { console.error('SoundCloud error:', err.message); return []; }),
        (await import('./lib/youtube')).searchYouTube(query, 10).catch((err) => { console.error('YouTube error:', err.message); return []; }),
        (await import('./lib/itunes')).searchItunes(query, 10).catch((err) => { console.error('iTunes error:', err.message); return []; }),
        (await import('./lib/internet-archive')).searchInternetArchive(query, 10).catch((err) => { console.error('Internet Archive error:', err.message); return []; }),
        (await import('./lib/mixcloud')).searchMixcloud(query, 10).catch((err) => { console.error('Mixcloud error:', err.message); return []; }),
      ]);
      ...
      console.log(`[SEARCH] Query: "${query}", Results before filtering: ${results.length}`);

      // Filter: only valid music results
      let filtered = results.filter((r: any) => {
        const url = (r.url || '').toLowerCase();
        const isMusicSite = url.includes('jamendo.com') || url.includes('youtube.com') || url.includes('youtu.be') || url.includes('soundcloud.com') || url.includes('deezer.com') || url.includes('apple.com') || url.includes('itunes.com') || url.includes('music.apple.com') || url.includes('archive.org') || url.includes('mixcloud.com');
